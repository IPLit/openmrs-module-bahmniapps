...
describe("condition", function () {
    ...
    describe("clearConcept", function () {
        it("should reset concept uuid", function () {
            var condition = new Condition({
                condition: {
                    coded: { uuid: 'uuid', display: 'Test Condition' }
                }
            });
    ...
    describe("isValid", function () {
        it("should be okay to have coded concept with a status", function () {
            var condition = new Condition({
                condition: {
                    coded: { uuid: 'uuid', display: 'concept name' }
                },
                isNonCoded: false,
    ...
    describe("displayString", function () {
        it("should be the name of non coded condition", function () {
            var condition = new Condition({
                condition: {
                    nonCoded: 'conditionNonCoded name'
                }
            });
        ...
        it("should be the short name of coded condition", function () {
            var condition = new Condition({
                condition: {
                    coded: {
                        uuid: 'uuid',
                        display: 'coded full name',
                        name: { name: 'coded short name' }
                    }
                }
            });
        ...
        it("should be the second previous condition when that is active", function () {
            var conditions = [
                new Condition({
                    ...
                    previousVersion: { uuid: '2nd condition uuid' }
                }),
                new Condition({
                    uuid: '2nd condition uuid',
                    previousVersion: { uuid: '3rd condition uuid' }
                }),
                new Condition({
                    status: 'ACTIVE',
    ...
    describe("fromConditionHistories", function () {
        it("should consider only condition which is not voided", function () {
            var conditionHistories = [
                {
                    uuid: '001',
                    condition: {
                        coded: { uuid: "Headache UUID", display: 'Headache' }
                    },
                    clinicalStatus: 'ACTIVE',
            ...
            var latestConditions = Conditions.fromConditionHistories(conditionHistories);
